...
- Webhooks (`src/routes/payments.ts`) dispatch by `session.metadata.type`: `"package"` → `activatePackagePurchase`, `"gift_card"` → `activateGiftCard`, default → booking flow. All branches are idempotent (skip if already PAID/ACTIVE). Amount is compared against DB; mismatches log a `WEBHOOK_INVALID_SIG` event and return 400.
